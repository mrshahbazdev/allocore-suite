                    ...
                    @foreach ($phases as $phase)
                        <details class="overflow-hidden rounded-lg border border-slate-200">
                            <summary class="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 hover:bg-slate-50">
                                <span class="font-medium text-slate-900">{{ __($phase['name']) }}</span>
                            ...
                            <ul class="divide-y divide-slate-100 border-t border-slate-100 bg-slate-50/60">
                                @foreach ($phase['questions'] as $question)
                                    <li class="flex items-start justify-between gap-4 px-4 py-2.5 text-sm">
                                        <span class="text-slate-700">{{ $question['position'] }}. {{ __($question['name']) }}</span>
                    ...
                    <section class="rounded-xl border border-slate-200 p-4">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <h5 class="mt-1 text-lg font-semibold text-slate-900">{{ __($phase['name']) }}</h5>
                            </div>
                        ...
                        <p class="mt-2 text-sm text-slate-500">{{ __($phase['description']) }}</p>
                        ...
                        <div class="mt-4 divide-y divide-slate-100 border-t border-slate-100">
                            @foreach ($phase['questions'] as $question)
                                <div class="py-3">
                                    <div class="flex items-start justify-between gap-4">
                                        <p class="text-sm font-medium text-slate-800">{{ $question['position'] }}. {{ __($question['name']) }}</p>
                                    ...
                                    <p class="mt-1 text-xs leading-5 text-slate-500">{{ __($question['description']) }}</p>
