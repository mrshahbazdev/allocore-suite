...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('orgmatrix.organizations.store') }}" class="space-y-5">
            @csrf
            <div>
            ...
            <div class="flex justify-end gap-3 pt-2">
                <a href="{{ route('orgmatrix.organizations.index') }}" class="rounded-lg bg-slate-100 px-5 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-200">{{ __('Cancel') }}</a>
                <button class="rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-indigo-500">{{ __('Create Organization') }}</button>
            </div>
        </form>
    </div>
</div>
