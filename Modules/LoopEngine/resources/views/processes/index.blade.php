        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <table class="min-w-full text-sm">
                <thead class="text-left text-xs uppercase tracking-wide text-slate-500">
                    <tr><th class="pb-2 pr-4">{{ __('Name') }}</th><th class="pb-2 pr-4">{{ __('Status') }}</th><th class="pb-2 pr-4">{{ __('Version') }}</th><th class="pb-2 pr-4">{{ __('Category') }}</th><th class="pb-2"></th></tr>
                </thead>
                <tbody class="divide-y divide-slate-200">
                    @foreach ($processes as $process)
                        <tr>
                            ...
                            <td class="py-2 pr-4 text-slate-500">{{ $process->category ?? '-' }}</td>
                            <td class="py-2 flex flex-wrap gap-2">
                                ...
                                <a href="{{ route('loopengine.processes.share', $process) }}" class="text-emerald-600">{{ __('Share') }}</a>
                                @if ($process->status !== 'active')
                                    <form method="POST" action="{{ route('loopengine.processes.activate', $process) }}" class="inline">@csrf<button class="text-emerald-600">{{ __('Activate') }}</button></form>
                                @endif
                                @if ($process->status !== 'archived')
                                    <form method="POST" action="{{ route('loopengine.processes.archive', $process) }}" class="inline">@csrf<button class="text-amber-600">{{ __('Archive') }}</button></form>
                                @endif
                                <form method="POST" action="{{ route('loopengine.processes.version', $process) }}" class="inline">@csrf<button class="text-slate-600">{{ __('New Version') }}</button></form>
                                <form method="POST" action="{{ route('loopengine.processes.destroy', $process) }}" class="inline" onsubmit="return confirm('{{ __('Delete this process?') }}')">@csrf @method('DELETE')<button class="text-rose-600">{{ __('Delete') }}</button></form>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
