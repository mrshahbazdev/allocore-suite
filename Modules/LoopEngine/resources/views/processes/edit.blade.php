        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form method="POST" action="{{ route('loopengine.steps.store', $process) }}" class="mt-4 space-y-3">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div><label class="block text-sm font-medium text-slate-700">{{ __('Question (EN)') }}</label><textarea name="question_en" rows="2" class="mt-1 w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500" required></textarea></div>
                    <div><label class="block text-sm font-medium text-slate-700">{{ __('Question (DE)') }}</label><textarea name="question_de" rows="2" class="mt-1 w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500"></textarea></div>
                </div>
                <div class="grid gap-4 sm:grid-cols-3">
                    ...
                    <div class="flex items-center gap-2 pt-6"><input type="checkbox" name="is_loop_checkpoint" value="1" class="rounded border-slate-300 focus:border-indigo-500 focus:ring-indigo-500"><span class="text-sm text-slate-700">{{ __('Loop checkpoint') }}</span></div>
                    <div class="flex items-center gap-2 pt-6"><input type="checkbox" name="is_required" value="1" checked class="rounded border-slate-300 focus:border-indigo-500 focus:ring-indigo-500"><span class="text-sm text-slate-700">{{ __('Required') }}</span></div>
                </div>
                <div class="grid gap-4 sm:grid-cols-2">
                    <div><label class="block text-sm font-medium text-slate-700">{{ __('Help (EN)') }}</label><input type="text" name="help_text_en" class="mt-1 w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500"></div>
                    <div><label class="block text-sm font-medium text-slate-700">{{ __('Help (DE)') }}</label><input type="text" name="help_text_de" class="mt-1 w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500"></div>
                    <div><label class="block text-sm font-medium text-slate-700">{{ __('Max Loops') }}</label><input type="number" name="max_loops" min="0" value="0" class="mt-1 w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500"></div>
                </div>
