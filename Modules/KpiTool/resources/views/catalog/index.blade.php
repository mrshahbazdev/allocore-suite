...
@section('title', __('KPI Catalog'))
@section('page-title', __('KPI Catalog'))

@section('content')
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-slate-900">{{ __('KPI Catalog') }}</h1>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($templates as $template)
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div class="mt-2 text-xs text-slate-500">{{ $template->formula }} — {{ $template->unit }}</div>
                    <form method="POST" action="{{ route('kpitool.catalog.clone', $template) }}" class="mt-4">
                        @csrf
                        <button class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">{{ __('Use KPI') }}</button>
                    </form>
                </div>
            @endforeach
        </div>
