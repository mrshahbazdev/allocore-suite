...
@section('content')
    <div class="space-y-6">
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between"><h2 class="text-lg font-semibold text-slate-900">{{ __('Tasks') }}</h2><div class="flex items-center gap-3"><a href="{{ route('planhive.tasks.index', $project) }}" class="text-xs text-slate-500 hover:text-indigo-600">{{ __('View all') }}</a><a href="{{ route('planhive.tasks.create', $project) }}" class="text-sm text-indigo-600">{{ __('New') }}</a></div></div>
                <ul class="mt-3 space-y-2 text-sm">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between"><h2 class="text-lg font-semibold text-slate-900">{{ __('Goals') }}</h2><div class="flex items-center gap-3"><a href="{{ route('planhive.goals.index', $project) }}" class="text-xs text-slate-500 hover:text-indigo-600">{{ __('View all') }}</a><a href="{{ route('planhive.goals.create', $project) }}" class="text-sm text-indigo-600">{{ __('New') }}</a></div></div>
                <ul class="mt-3 space-y-2 text-sm">
                    @forelse ($project->goals as $goal)
                        <li>
                            <div class="flex items-center justify-between"><a href="{{ route('planhive.goals.show', $goal) }}" class="text-indigo-600 hover:underline">{{ $goal->title }}</a><span class="text-xs text-slate-500">{{ $goal->progress }}%</span></div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between"><h2 class="text-lg font-semibold text-slate-900">{{ __('Notes') }}</h2><div class="flex items-center gap-3"><a href="{{ route('planhive.notes.index', $project) }}" class="text-xs text-slate-500 hover:text-indigo-600">{{ __('View all') }}</a><a href="{{ route('planhive.notes.create', $project) }}" class="text-sm text-indigo-600">{{ __('New') }}</a></div></div>
                <ul class="mt-3 space-y-2 text-sm">
                    @forelse ($project->notes as $note)
                        <li><a href="{{ route('planhive.notes.show', $note) }}" class="text-indigo-600 hover:underline">{{ $note->title }}</a></li>
                    @empty
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between"><h2 class="text-lg font-semibold text-slate-900">{{ __('Contacts') }}</h2><div class="flex items-center gap-3"><a href="{{ route('planhive.contacts.index', $project) }}" class="text-xs text-slate-500 hover:text-indigo-600">{{ __('View all') }}</a><a href="{{ route('planhive.contacts.create', $project) }}" class="text-sm text-indigo-600">{{ __('New') }}</a></div></div>
                <ul class="mt-3 space-y-2 text-sm">
                    @forelse ($project->contacts as $contact)
                        <li class="flex justify-between"><a href="{{ route('planhive.contacts.show', $contact) }}" class="text-indigo-600 hover:underline">{{ $contact->name }}</a><span class="text-slate-500">{{ $contact->company }}</span></li>
                    @empty
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <ul class="mt-3 space-y-2 text-sm">
                    @forelse ($project->calendarEvents->take(5) as $event)
                        <li class="flex justify-between"><a href="{{ route('planhive.calendar-events.show', $event) }}" class="text-indigo-600 hover:underline">{{ $event->title }}</a><span class="text-xs text-slate-500">{{ $event->start_at->format('M d H:i') }}</span></li>
                    @empty
