...
<div class="import-layout">
  ...
  {{-- Upload Form --}}
  <div class="card">
    <div class="card-title">{{ __('📤 Excel-Datei hochladen') }}</div>

    <form method="POST" action="{{ route('import.upload') }}" enctype="multipart/form-data">
    @csrf
    <div class="form-group">
      <label class="form-label">{{ __('Analyse-Typ *') }}</label>
      <select name="type" id="import-type" class="form-control" required onchange="updateHint(this.value)">
        <option value="">{{ __('— Bitte wählen —') }}</option>
        <option value="gmbh">{{ __('📊 GmbH Analyse') }}</option>
        <option value="jahresabschluss">{{ __('📈 Jahresabschluss (bis 3 Jahre)') }}</option>
        <option value="immobilien">{{ __('🏘 Immobilienanalyse') }}</option>
      </select>
    </div>
    ...
    <div class="form-group">
      ...
      <input type="text" name="name" class="form-control" placeholder="{{ __('Excel Import 2024') }}" required>
    </div>
    ...
    <div class="form-group">
      ...
      <div id="drop-zone" style="border:2px dashed rgba(99,102,241,0.3); border-radius:12px; padding:40px 20px; text-align:center; cursor:pointer; transition:all .2s; position:relative;"
        ...
        <div style="font-size:14px; color:#94a3b8; margin-bottom:4px;">{{ __('Datei hier hinziehen') }}</div>
  ...
  <div class="import-side-stack">
    ...
    <div class="card" id="hint-box">
      <div class="card-title" id="hint-title">{{ __('📋 Spalteninfo') }}</div>
      <div id="hint-gmbh" style="display:none;">
        <div class="import-hint-table-wrap">
        <table class="data-table" style="font-size:11px;">
          <thead><tr><th>{{ __('Spalte') }}</th><th>{{ __('Feldname') }}</th></tr></thead>
          <tbody>
      ...
      <div id="hint-jahresabschluss" style="display:none;">
        <div style="font-size:11px; color:#64748b; line-height:1.7;">{{ __('Jede') }}<strong style="color:#fbbf24;">Zeile = 1 Jahr</strong>{{ __('(max. 3 Zeilen).') }}<br>
      ...
      <div id="hint-immobilien" style="display:none;">
        <div style="font-size:11px; color:#64748b; line-height:1.7;">
          <strong style="color:#c084fc;">{{ __('Eine Zeile') }}</strong>{{ __('pro Objekt.') }}<br>
      ...
      <div id="hint-default" style="font-size:12px; color:#475569;">{{ __('Wählen Sie einen Typ, um die Spalteninfo zu sehen.') }}</div>
    </div>

    {{-- Instructions --}}
    <div class="card">
      <div class="card-title">{{ __("ℹ️ So funktioniert's") }}</div>
