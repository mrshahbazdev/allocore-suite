...
@section('topbar-actions')
    <a href="{{ route('companies.show', $company) }}" class="btn btn-secondary btn-sm">{{ __('← Zurück') }}</a>
@endsection
@push('styles')
<style>
...
<div class="card">
    ...
    <div class="form-grid">
        ...
        <div class="form-group">
            ...
            <select name="currency" class="form-control">
                <option value="EUR" {{ old('currency',$company->currency)==='EUR'?'selected':'' }}>{{ __('EUR €') }}</option>
                <option value="USD" {{ old('currency',$company->currency)==='USD'?'selected':'' }}>USD $</option>
                <option value="CHF" {{ old('currency',$company->currency)==='CHF'?'selected':'' }}>{{ __('CHF') }}</option>
            </select>
        </div>
    </div>
    <div class="form-group">
    ...
    <button type="submit" class="btn btn-primary" style="width:100%; justify-content:center;">{{ __('💾 Speichern') }}</button>
</div>
</form>
</div>
@endsection
