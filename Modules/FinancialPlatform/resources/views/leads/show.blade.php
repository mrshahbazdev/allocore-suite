...
@section('topbar-actions')
    <a href="{{ route('leads.edit', $lead) }}" class="btn btn-secondary btn-sm">{{ __('Bearbeiten') }}</a>
...
<div class="lead-detail-grid">
    {{-- Main info --}}
    <div class="card">
        <div class="card-title">{{ __('Lead-Details') }}</div>
        ...
        <div class="detail-row">
            <span class="detail-label">{{ __('Telefon') }}</span>
            <span class="detail-value">{{ $lead->phone ?? '—' }}</span>
        </div>
        <div class="detail-row">
            <span class="detail-label">{{ __('Position') }}</span>
            <span class="detail-value">{{ $lead->position ?? '—' }}</span>
        </div>
        <div class="detail-row">
            <span class="detail-label">{{ __('Unternehmen') }}</span>
            <span class="detail-value">{{ $lead->company_name ?? $lead->company?->name ?? '—' }}</span>
        </div>
        <div class="detail-row">
            <span class="detail-label">{{ __('Branche') }}</span>
            <span class="detail-value">{{ $lead->industry ?? '—' }}</span>
        </div>
        <div class="detail-row">
            <span class="detail-label">{{ __('LinkedIn') }}</span>
            <span class="detail-value">
                @if($lead->linkedin)
                    <a href="{{ $lead->linkedin }}" target="_blank" style="color:#818cf8;">{{ __('Profil öffnen') }}</a>
                @else — @endif
            </span>
        </div>
        <div class="detail-row">
            <span class="detail-label">{{ __('Website') }}</span>
            <span class="detail-value">
                @if($lead->website)
        ...
        <div class="detail-row">
            <span class="detail-label">{{ __('Budget') }}</span>
    ...
    <div style="display:flex; flex-direction:column; gap:16px;">
        ...
        {{-- LeadOS transfer --}}
        <div class="card">
            <div class="card-title">{{ __('LeadOS Integration') }}</div>
            @if($lead->transferred_to_leados)
                <div style="text-align:center; padding:12px;">
                    <span class="badge badge-green" style="font-size:13px; padding:6px 14px;">{{ __('An LeadOS übertragen') }}</span>
            ...
            @else
                <form method="POST" action="{{ route('leads.transfer') }}">
                    ...
                    <div class="form-group">
                        <label class="form-label">{{ __('LeadOS API-URL') }}</label>
                        <input type="url" name="leados_api_url" class="form-control" placeholder="{{ __('https://lead.os') }}" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">{{ __('API-Token') }}</label>
                        <input type="text" name="leados_token" class="form-control" placeholder="{{ __('Bearer Token') }}" required>
                    </div>
                    <button type="submit" class="btn btn-success btn-sm" style="width:100%; justify-content:center;">{{ __('An LeadOS übertragen') }}</button>
                </form>
            @endif
        </div>
        ...
        <div class="card">
            <div class="card-title">{{ __('PayPal-Transaktionen') }}</div>
        ...
        {{-- Actions --}}
        <div class="card">
            <div class="card-title">{{ __('Aktionen') }}</div>
            <div style="display:flex; flex-direction:column; gap:8px;">
                <a href="{{ route('leads.edit', $lead) }}" class="btn btn-secondary btn-sm" style="justify-content:center;">{{ __('Bearbeiten') }}</a>
                <form method="POST" action="{{ route('leads.destroy', $lead) }}" onsubmit="return confirm('Lead wirklich löschen?')">
                    @csrf @method('DELETE')
                    <button type="submit" class="btn btn-danger btn-sm" style="width:100%; justify-content:center;">{{ __('Löschen') }}</button>
                </form>
            </div>
        </div>
    </div>
