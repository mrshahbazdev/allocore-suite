...
@section('topbar-actions')
    <a href="{{ route('paypal.settings') }}" class="btn btn-secondary btn-sm">{{ __('Einstellungen') }}</a>
@endsection
...
<div class="card new-payment-card">
    ...
    <form method="POST" action="{{ route('paypal.create-payment') }}" class="payment-form-row">
        ...
        <div class="form-group">
            ...
            <input type="text" name="description" class="form-control" placeholder="{{ __('z.B. Beratungsgebühr') }}" style="min-width:200px;">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <button type="submit" class="btn btn-primary">{{ __('Mit PayPal bezahlen') }}</button>
        </div>
    </form>
</div>
...
<div class="card">
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:16px;">
        <div class="card-title" style="margin-bottom:0">{{ __('Transaktionen') }}</div>
    ...
    @if($transactions->isEmpty())
        <div style="text-align:center; padding:40px; color:#475569;">
            ...
            <div style="font-size:14px; margin-bottom:8px;">{{ __('Noch keine PayPal-Transaktionen') }}</div>
            <div style="font-size:12px; color:#334155;">{{ __('Erstellen Sie eine Zahlung oder konfigurieren Sie PayPal') }}</div>
        </div>
    @else
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="{{ route('paypal.show', $txn) }}" class="btn btn-secondary btn-sm">{{ __('Details') }}</a></td>
                </tr>
                @endforeach
            </tbody>
        </table>
