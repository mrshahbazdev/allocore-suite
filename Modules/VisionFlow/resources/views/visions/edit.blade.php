...
<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-slate-900 mb-6">{{ __('Edit Vision') }}</h1>
...
@endif
        ...
        <form method="POST" action="{{ route('visionflow.organizations.visions.update', [$organization, $item]) }}" class="space-y-4">
            @csrf @method('PATCH')
            <div>
    <label class="block text-sm font-medium text-slate-700">{{ __('Content') }}</label>
    <textarea name="content" rows="3" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm" required>{{ $item->content }}</textarea>
</div>
<div>
    ...
    <select name="status" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm" required>
        <option value="drafting" {{ $item->status == "drafting" ? "selected" : "" }}>{{ __('drafting') }}</option><option value="reviewing" {{ $item->status == "reviewing" ? "selected" : "" }}>{{ __('reviewing') }}</option><option value="approved" {{ $item->status == "approved" ? "selected" : "" }}>{{ __('approved') }}</option>
    </select>
</div>
<div>
    <label class="block text-sm font-medium text-slate-700">{{ __('Version') }}</label>
    <input type="number" name="version" value="$item->version" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm">
</div>
