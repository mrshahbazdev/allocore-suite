...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        ...
        <a href="{{ route('visionflow.organizations.visions.index', $organization) }}" class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
        <div><div class="text-sm text-slate-500">{{ __("Content") }}</div><div class="mt-1 text-slate-900">{{ $item->content }}</div></div>
        ...
        <div><div class="text-sm text-slate-500">{{ __("Version") }}</div><div class="mt-1 text-slate-900">{{ $item->version }}</div></div>
        <div><div class="text-sm text-slate-500">{{ __("Is Current") }}</div><div class="mt-1 text-slate-900">{{ $item->is_current }}</div></div>
    </div>
</div>
@endsection
