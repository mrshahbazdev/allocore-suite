...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        ...
        <a href="{{ route('visionflow.organizations.strategic-goals.index', $organization) }}" class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
        ...
        <div><div class="text-sm text-slate-500">{{ __("Category") }}</div><div class="mt-1"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium {{ strtolower($item->category) == 'approved' || strtolower($item->category) == 'active' || strtolower($item->category) == 'completed' || strtolower($item->category) == 'current' ? 'bg-emerald-100 text-emerald-700' : (strtolower($item->category) == 'archived' || strtolower($item->category) == 'on_hold' || strtolower($item->category) == 'paused' ? 'bg-slate-100 text-slate-600' : (strtolower($item->category) == 'proposed' || strtolower($item->category) == 'reviewing' || strtolower($item->category) == 'drafting' ? 'bg-amber-100 text-amber-700' : (strtolower($item->category) == 'draft' ? 'bg-slate-100 text-slate-600' : 'bg-slate-100 text-slate-700')))) }}">{{ $item->category }}</span></div></div>
        <div><div class="text-sm text-slate-500">{{ __("Time Horizon") }}</div><div class="mt-1 text-slate-900">{{ $item->time_horizon }}</div></div>
