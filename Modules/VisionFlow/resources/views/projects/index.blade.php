@extends('layouts.shell', ['title' => __('Projects')])
...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex flex-wrap items-center gap-2 text-sm text-slate-500">
        ...
        <span>/</span>
        <span class="text-slate-900">{{ __('Projects') }}</span>
    </div>

    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <h1 class="text-3xl font-bold text-slate-900">{{ __('Projects') }}</h1>
        <a href="{{ route('visionflow.organizations.projects.create', $organization) }}" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
    ...
    @if ($items->isEmpty())
        <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-12 text-center">
            ...
            <h3 class="mt-4 text-lg font-semibold text-slate-900">{{ __('No projects yet') }}</h3>
            <p class="mt-1 text-sm text-slate-500">{{ __('Create your first project to build your organization alignment.') }}</p>
        </div>
    @else
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __("Name") }}</th>
                ...
                <tbody class="divide-y divide-slate-200">
                    @foreach ($items as $item)
                        <tr class="transition hover:bg-slate-50">
                            <td class="px-5 py-4 text-sm text-slate-900">{{ $item->name }}</td>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a href="{{ route('visionflow.organizations.projects.edit', [$organization, $item]) }}" class="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-200" title="{{ __('Edit') }}">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('visionflow.organizations.projects.destroy', [$organization, $item]) }}" onsubmit="return confirm('{{ __('Delete this project?') }}')" class="inline">
                                        @csrf @method('DELETE')
