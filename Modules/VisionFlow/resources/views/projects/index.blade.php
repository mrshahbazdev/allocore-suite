...
@endif
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('visionflow.organizations.projects.create', $organization) }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">{{ __('New') }}</a>
    </div>
    @if ($items->isEmpty())
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <table class="min-w-full text-left text-sm">
                <thead class="bg-slate-50 text-slate-700 font-medium">
                    <tr>
                        <th class="px-4 py-3">{{ __("Name") }}</th><th class="px-4 py-3">{{ __("Mission") }}</th><th class="px-4 py-3">{{ __("Status") }}</th>
                    ...
                    @foreach ($items as $item)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 flex gap-3">
                                ...
                                <form method="POST" action="{{ route('visionflow.organizations.projects.destroy', [$organization, $item]) }}" onsubmit="return confirm('{{ __('Delete?') }}')" class="inline">
                                    @csrf @method('DELETE')
