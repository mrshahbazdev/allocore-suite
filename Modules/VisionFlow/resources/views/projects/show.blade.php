...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        ...
        <a href="{{ route('visionflow.organizations.projects.index', $organization) }}" class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
        <div><div class="text-sm text-slate-500">{{ __("Name") }}</div><div class="mt-1 text-slate-900">{{ $item->name }}</div></div>
        <div><div class="text-sm text-slate-500">{{ __("Mission") }}</div><div class="mt-1 text-slate-900">{{ $item->mission->title ?? '-' }}</div></div>
