...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex flex-wrap items-center gap-2 text-sm text-slate-500">
        ...
        <span>/</span>
        <span class="text-slate-900">{{ __('Edit Project') }}</span>
    </div>

    <h1 class="text-3xl font-bold text-slate-900">{{ __('Edit Project') }}</h1>

    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('visionflow.organizations.projects.update', [$organization, $item]) }}" class="space-y-5">
            @csrf @method('PATCH')
            <div>
...
<div>
    ...
    <input type="text" name="name" value="{{ $item->name }}" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
</div>
<div>
...
<div>
    ...
    <select name="status" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
        ...
        <option value="on_hold" {{ $item->status == 'on_hold' ? 'selected' : '' }}>{{ __("on_hold") }}</option>
        ...
        <option value="archived" {{ $item->status == 'archived' ? 'selected' : '' }}>{{ __("archived") }}</option>
            ...
            <div class="flex justify-end gap-3 pt-2">
                ...
                <button class="rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">{{ __('Update Project') }}</button>
            </div>
        </form>
    </div>
</div>
