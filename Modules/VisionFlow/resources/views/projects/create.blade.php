...
<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-slate-900 mb-6">{{ __('New Project') }}</h1>
...
@endif
        ...
        <form method="POST" action="{{ route('visionflow.organizations.projects.store', $organization) }}" class="space-y-4">
            @csrf
            <div>
    ...
    <select name="mission_id" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm" required>
        ...
        @foreach ($missions as $id => $label)
            <option value="{{ $id }}" {{ $->mission_id == $id ? 'selected' : '' }}>{{ $label }}</option>
        @endforeach
    </select>
</div>
<div>
    <label class="block text-sm font-medium text-slate-700">{{ __('Name') }}</label>
    <input type="text" name="name" value="''" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm" required>
</div>
<div>
...
<div>
    ...
    <select name="status" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm" required>
        <option value="active" {{ null == "active" ? "selected" : "" }}>{{ __('active') }}</option><option value="on_hold" {{ null == "on_hold" ? "selected" : "" }}>{{ __('on_hold') }}</option><option value="completed" {{ null == "completed" ? "selected" : "" }}>{{ __('completed') }}</option><option value="archived" {{ null == "archived" ? "selected" : "" }}>{{ __('archived') }}</option>
    </select>
</div>
