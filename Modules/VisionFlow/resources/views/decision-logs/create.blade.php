...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex flex-wrap items-center gap-2 text-sm text-slate-500">
        ...
        <span>/</span>
        <span class="text-slate-900">{{ __('New Decision Log') }}</span>
    </div>

    <h1 class="text-3xl font-bold text-slate-900">{{ __('New Decision Log') }}</h1>

    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('visionflow.organizations.decision-logs.store', $organization) }}" class="space-y-5">
            @csrf
            <div>
...
<div>
    ...
    <textarea name="decision" rows="4" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required></textarea>
</div>
<div>
    ...
    <select name="supporting_mission_id" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        ...
        @endforeach
            ...
            <div class="flex justify-end gap-3 pt-2">
                ...
                <button class="rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">{{ __('Create Decision Log') }}</button>
            </div>
        </form>
    </div>
</div>
