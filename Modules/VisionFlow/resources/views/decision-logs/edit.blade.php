...
<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-slate-900 mb-6">{{ __('Edit Decision Log') }}</h1>
...
@endif
        ...
        <form method="POST" action="{{ route('visionflow.organizations.decision-logs.update', [$organization, $item]) }}" class="space-y-4">
            @csrf @method('PATCH')
            <div>
...
<div>
    ...
    <textarea name="decision" rows="3" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm" required>{{ $item->decision }}</textarea>
</div>
<div>
