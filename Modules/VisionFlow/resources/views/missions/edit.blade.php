...
<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-slate-900 mb-6">{{ __('Edit Mission') }}</h1>
...
@endif
        ...
        <form method="POST" action="{{ route('visionflow.organizations.missions.update', [$organization, $item]) }}" class="space-y-4">
            @csrf @method('PATCH')
            <div>
...
<div>
    ...
    <select name="status" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm" required>
        <option value="active" {{ $item->status == "active" ? "selected" : "" }}>{{ __('active') }}</option><option value="paused" {{ $item->status == "paused" ? "selected" : "" }}>{{ __('paused') }}</option><option value="completed" {{ $item->status == "completed" ? "selected" : "" }}>{{ __('completed') }}</option><option value="archived" {{ $item->status == "archived" ? "selected" : "" }}>{{ __('archived') }}</option>
    </select>
</div>
<div>
    ...
    <select name="review_cadence" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm">
        <option value="monthly" {{ $item->review_cadence == "monthly" ? "selected" : "" }}>{{ __('monthly') }}</option><option value="quarterly" {{ $item->review_cadence == "quarterly" ? "selected" : "" }}>{{ __('quarterly') }}</option><option value="biannually" {{ $item->review_cadence == "biannually" ? "selected" : "" }}>{{ __('biannually') }}</option><option value="annually" {{ $item->review_cadence == "annually" ? "selected" : "" }}>{{ __('annually') }}</option>
    </select>
</div>
<div>
    <label class="block text-sm font-medium text-slate-700">{{ __('Next Review') }}</label>
    <input type="date" name="next_review_at" value="$item->next_review_at" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm">
</div>
