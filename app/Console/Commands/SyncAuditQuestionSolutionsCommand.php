...
class SyncAuditQuestionSolutionsCommand extends Command
{
    protected $signature = 'audit:sync-solutions';

    protected $description = 'Synchronize recommended tools, modules, and glossary terms for all audit questions';

    public function handle(): int
    {
        $this->info('Starting audit question solutions synchronization...');
        ...
        foreach ($mappings as $questionText => $data) {
            ...
            $affected = DB::table('auditpro_questions')
        ...
        $leadAffected = DB::table('auditpro_questions')
            ->where(function ($q) {
        ...
        $updatedCount += $leadAffected;

        $this->info("Successfully synchronized {$updatedCount} question assignments across all templates.");
