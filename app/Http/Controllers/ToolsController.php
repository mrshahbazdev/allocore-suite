...
{
    public function __invoke(Request $request, ModuleRecommender $recommender, ModuleStats $moduleStats)
    {
        $user = $request->user();
        $modules = Module::where('is_active', true)->orderBy('name')->get()
            ->filter(fn (Module $module) => $moduleStats->modelFor($module->key) !== null)
            ->values();
