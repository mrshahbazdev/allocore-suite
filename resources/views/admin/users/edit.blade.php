    ...
    <div class="max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-5">
            ...
            <div>
                ...
                @php($userRoles = $user->roles->pluck('name')->toArray())
                <div class="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($roles as $role)
                        <label class="flex items-center gap-2 rounded-lg border border-slate-200 p-2 hover:bg-slate-50">
                            <input type="checkbox" name="roles[]" value="{{ $role->name }}" {{ in_array($role->name, old('roles', $userRoles), true) ? 'checked' : '' }} class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div class="flex items-center gap-2">
                    <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" @checked(old('is_active', $user->is_active))>
            ...
            <div>
                ...
                <select name="locale" class="mt-2 block w-full rounded-lg border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    @foreach (config('app.available_locales', ['en']) as $locale)
                        <option value="{{ $locale }}" @selected(old('locale', $user->locale ?? 'en') === $locale)>{{ strtoupper($locale) }}</option>
                    @endforeach
                </select>
            </div>
