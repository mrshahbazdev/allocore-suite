...
<div class="grid gap-5 md:grid-cols-2">
    <div>
        ...
        <input name="slug" value="{{ old('slug', $glossary->slug ?? '') }}" class="mt-2 block w-full rounded-lg border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="{{ __('auto-generated') }}">
    </div>
    <div>
        <label class="block text-sm font-medium text-slate-700">{{ __('Category') }}</label>
        <input name="category" value="{{ old('category', $glossary->category ?? '') }}" class="mt-2 block w-full rounded-lg border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="{{ __('e.g. Finance, Strategy') }}">
    </div>
</div>
...
<div>
    ...
    <input name="related_modules" value="{{ old('related_modules', isset($glossary) ? collect($glossary->related_modules ?? [])->implode(', ') : '') }}" class="mt-2 block w-full rounded-lg border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="{{ __('financial-platform, sweet-spot') }}">
