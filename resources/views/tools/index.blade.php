            ...
            <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                @foreach (array_slice($recommendations['combos'], 0, 3) as $combo)
                    <div class="rounded-lg bg-white border border-indigo-100 p-4">
                        <h3 class="font-semibold text-slate-900 break-words">{{ $combo['suggest_name'] }}</h3>
                        <p class="mt-1 text-xs text-slate-500 break-words">{{ $combo['reason'] }}</p>
                ...
                @foreach (array_slice($recommendations['similar'], 0, 3 - count(array_slice($recommendations['combos'], 0, 3))) as $module)
                    <div class="rounded-lg bg-white border border-slate-200 p-4">
                        ...
                        <p class="mt-1 text-xs text-slate-500 break-words">{{ $module['reason'] }}</p>
            ...
            @foreach ($accessible as $module)
                <div class="rounded-xl bg-white border border-slate-200 p-5 shadow-sm">
                    <div class="flex items-start justify-between gap-2">
                        <h3 class="font-semibold text-slate-900 break-words">{{ $module->name }}</h3>
                        <span class="shrink-0 text-[10px] rounded-full bg-emerald-100 text-emerald-700 px-2 py-0.5 font-medium">{{ __('Active') }}</span>
                    </div>
                    <p class="mt-1 text-sm text-slate-500 break-words">{{ Str::limit($module->description, 80) }}</p>
                    <div class="mt-4">
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            @foreach ($locked as $module)
                <div class="rounded-xl bg-white border border-slate-200 p-5 shadow-sm opacity-90">
                    <div class="flex items-start justify-between gap-2">
                        <h3 class="font-semibold text-slate-900 break-words">{{ $module->name }}</h3>
                        <span class="shrink-0 text-[10px] rounded-full bg-slate-100 text-slate-500 px-2 py-0.5 font-medium">{{ __('Locked') }}</span>
                    </div>
                    <p class="mt-1 text-sm text-slate-500 break-words">{{ Str::limit($module->description, 80) }}</p>
                    <div class="mt-4">
