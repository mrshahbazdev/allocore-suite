...
@section('meta_description', __('Posts tagged with :name', ['name' => $tag->name]))

@section('content')
<div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    <h1 class="mb-8 text-3xl font-bold text-slate-900">#{{ $tag->name }}</h1>

    @if ($posts->isEmpty())
        <div class="rounded-xl border border-slate-200 bg-white p-8 text-center text-slate-500">{{ __('No posts with this tag.') }}</div>
    @else
